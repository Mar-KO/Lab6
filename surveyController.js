...
*/
        ...
        var flaws = readData("flaws");
        var comment = readData("comment");
        var field = readData("field"); 
        var recommendation = readData("recommendation");

        res.render('showResults', {results: [like, gender, flaws, comment , field, recommendation]});
        console.log([like, gender, flaws, comment , field, recommendation]);
    });

    // when a user goes to localhost:3000/niceSurvey
    // serve a static html (the survey itself to fill in)
    app.get('/Lab1', function(req, res){
        res.sendFile(__dirname+'/views/Lab1.html');
    });

    // when a user types SUBMIT in localhost:3000/niceSurvey 
    // the action.js code will POST, and what is sent in the POST
    // will be recuperated here, parsed and used to update the data files
    app.post('/Lab1', urlencodedParser, function(req, res){
